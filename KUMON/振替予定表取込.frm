...
Begin VB.Form frmYoteiReqImport 
   ...
   Begin FPSpread.vaSpread sprDetail 
      ...
      MaxRows         =   15
      ScrollBars      =   2
      SpreadDesigner  =   "振替予定表取込.frx":07ED
      UserResize      =   0
   ...
   End
End
Attribute VB_Name = "frmYoteiReqImport"
...
#End If
#Const SPR_COL_HIDE = True              '//Spreadの非表示列を制御：デバック時のみ
...
#End If
    '// WHERE 句には必ず付加
    Dim SameConditions As String, SameConditionsNonDelete As String
    SameConditionsNonDelete = " AND FIINDT = TO_DATE('" & vImpDate & "','yyyy/mm/dd hh24:mi:ss')"
'//2006/06/16 明細削除対応
    SameConditions = SameConditionsNonDelete & vbCrLf & " AND FIERROR <> " & mYimp.errDeleted
    ...
    '//2007/03/12 合計の削除行は明細を全件削除：個別に解除した場合、解除されてしまうのを回避
    sql = "UPDATE " & mYimp.TfFurikaeImport & " a SET " & vbCrLf
    sql = sql & " FIOKFG  = " & mYimp.updDeleted & "," & vbCrLf
    ...
    sql = sql & "       WHERE 1 = 1" & vbCrLf  '//おまじない
    sql = sql & "       " & SameConditionsNonDelete & vbCrLf
    sql = sql & "         AND FIRKBN = " & mYimp.RecordIsTotal & vbCrLf
    sql = sql & "         AND FIERROR = " & mYimp.errDeleted & vbCrLf
    sql = sql & "       )"
    recCnt = gdDBS.Database.ExecuteSQL(sql)
    '//2007/03/12 削除行のＯＫフラグをクリア
    sql = "UPDATE " & mYimp.TfFurikaeImport & " a SET " & vbCrLf
    sql = sql & " FIOKFG = " & mYimp.updDeleted & "," & vbCrLf
    'sql = sql & mYimp.StatusColumns(" = " & mYimp.errNormal & "," & vbCrLf)
    sql = sql & " FIUSID = '" & gdDBS.LoginUserName & "'," & vbCrLf
    ...
    sql = sql & " WHERE 1 = 1" & vbCrLf  '//おまじない
    sql = sql & SameConditionsNonDelete & vbCrLf
    sql = sql & "  AND FIERROR = " & mYimp.errDeleted
    recCnt = gdDBS.Database.ExecuteSQL(sql)
    '//////////////////////////////////////////////////
...
#If 1 Then
    sql = "UPDATE " & mYimp.TfFurikaeImport & " a SET " & vbCrLf
    sql = sql & " FIKSCDE = " & mYimp.errInvalid & "," & vbCrLf
    sql = sql & " FIUSID = '" & gdDBS.LoginUserName & "'," & vbCrLf
    sql = sql & " FIUPDT = SYSDATE" & vbCrLf
    sql = sql & " WHERE (FIITKB,FIKYCD,FIKSCD) NOT IN(" & vbCrLf
    sql = sql & "   SELECT CAITKB,CAKYCD,CAKSCD " & vbCrLf
...
#End If
    recCnt = gdDBS.Database.ExecuteSQL(sql)
'//2007/03/09 ページ番号「0」をエラーとして追加
    sql = "UPDATE " & mYimp.TfFurikaeImport & " a SET " & vbCrLf
    sql = sql & " FIPGNOE = " & mYimp.errInvalid & "," & vbCrLf
    sql = sql & " FIUSID = '" & gdDBS.LoginUserName & "'," & vbCrLf
    sql = sql & " FIUPDT = SYSDATE" & vbCrLf
    sql = sql & " WHERE NVL(FIPGNO,0) = 0 " & vbCrLf
    sql = sql & SameConditions & vbCrLf
    recCnt = gdDBS.Database.ExecuteSQL(sql)
    '//////////////////////////////////////////////////
    '//保護者コード：有無
...
#End If
    ...
    recCnt = gdDBS.Database.ExecuteSQL(sql)
    '//////////////////////////////////////////////////
    '//保護者コード：振替予定データ：その１ 2007/06/13 (FIFKDTE / FIHGCDE) が一緒だったのを別にした＆教室結合追加
    '//////////////////////////////////////////////////
    If False = pProgressBarSet(Block) Then
        GoTo gDataCheckError:
    End If
    ...
    sql = sql & "              AND FAKSCD = a.FIKSCD " & vbCrLf '//2007/06/13 教室結合を追加
    ...
    recCnt = gdDBS.Database.ExecuteSQL(sql)
    '//////////////////////////////////////////////////
    '//保護者コード：振替予定データ：その２ 2007/06/13 (FIFKDTE / FIHGCDE) が一緒だったのを別にした＆教室結合追加
    '//////////////////////////////////////////////////
    If False = pProgressBarSet(Block) Then
        GoTo gDataCheckError:
    End If
...
#End If
        ...
        If Val(dynM.Fields("FIOKFG")) = mYimp.errNormal And Val(dynS.Fields("FIOKFG")) <> mYimp.errNormal Then
            '//2007/03/12 明細削除は合計行に正常フラグ設定
            If Val(dynS.Fields("FIOKFG")) = mYimp.updDeleted Then
                okFlag = mYimp.errNormal
            Else
...
Private Sub cmdCheck_Click()
    ...
    '//2007/03/12 チェック処理後のみマスタ反映可能に制御
...
Private Sub cmdDelete_Click()
    ...
    '//マスタ反映時にも同じ事をするので共通化
    recCnt = pDeletedMoveTempRecords(gcYoteiImportToDelete)
    If recCnt < 0 Then
...
'//2007/03/08 編集不可能な列を色変更
'''    mSprTotal.BackColor(eSprTotal.eImportCnt, -1) = vbRed
'''    mSprTotal.BackColor(eSprTotal.eMochikomiBi, -1) = vbRed
'''    mSprTotal.BackColor(eSprTotal.eKeiyakuName, -1) = vbRed
...
#End If
    sql = "SELECT FIERROR FROM " & mYimp.TfFurikaeImport
    sql = sql & " WHERE FIINDT = TO_DATE('" & cboImpDate.Text & "','yyyy/mm/dd hh24:mi:ss')" & vbCrLf
    sql = sql & "  AND FIERROR = " & mYimp.errEditData
#If ORA_DEBUG = 1 Then
...
#Else
    Set dyn = gdDBS.OpenRecordset(sql, OracleConstantModule.ORADYN_READONLY)
#End If
'//2007/03/08 削除行を計算対象外にしたため NULL が存在、０表示に！
    lblDetailCount.Caption = Format(gdDBS.Nz(dyn.Fields("FIHKCT"), 0), "#,##0")
    lblDetailKingaku.Caption = Format(gdDBS.Nz(dyn.Fields("FIHKKG"), 0), "#,##0")
    lblDetailCancel.Caption = Format(gdDBS.Nz(dyn.Fields("FIKYCT"), 0), "#,##0")
    Call dyn.Close
    Set dyn = Nothing
...
Private Sub cmdErrList_Click()
    ...
    Dim sql As String
    Load rptYoteiReqImport
    With rptYoteiReqImport
...
#End If
    ...
    sql = sql & " WHERE FIINDT = TO_DATE('" & cboImpDate.Text & "','yyyy/mm/dd hh24:mi:ss') " & vbCrLf
    sql = sql & "   AND(" & vbCrLf
    sql = sql & "           (" & cInSQLString & ") NOT IN(" & vbCrLf
    sql = sql & "     SELECT " & cInSQLString & vbCrLf
    sql = sql & "     FROM " & mYimp.TfFurikaeImport & " b " & vbCrLf
    sql = sql & "     WHERE a.FIINDT = b.FIINDT" & vbCrLf
'警告時と削除時のマスタ反映を見直し: おかしい 警告＝b.FIOKFG 削除＝a.FIOKFG　/ もともとは b.FIOKFG
    sql = sql & "       AND b.FIOKFG <> " & mYimp.updNormal & vbCrLf      '//正常でない：NOT IN
    sql = sql & "    )" & vbCrLf
'//2006/06/16 明細削除対応
    sql = sql & "      OR       (" & cInSQLString & ") IN(" & vbCrLf
    sql = sql & "         SELECT " & cInSQLString & vbCrLf
    sql = sql & "         FROM tfFurikaeYoteiImport b" & vbCrLf
    sql = sql & "         Where a.FIINDT = b.FIINDT" & vbCrLf
    sql = sql & "           AND b.FIOKFG = " & mYimp.updNormal & vbCrLf
    sql = sql & "           AND b.FIRKBN = " & mYimp.RecordIsTotal & vbCrLf '//合計レコードは不要
    sql = sql & "        )" & vbCrLf
    sql = sql & "    )" & vbCrLf
    sql = sql & "  AND FIERROR >= " & mYimp.errNormal & vbCrLf
...
'//2007/06/11 大量に AutoLog にかかれるのでトリガを停止
    ...
    Call gdDBS.TriggerControl("tfFurikaeYoteiData", False)
    fraProgressBar.Visible = True
...
'frmKouzaFurikaeExport(口座振替データ作成) でのデータは FASKGK を出力している！
        ...
        sql = sql & " FAUSID = " & gdDBS.ColumnDataSet(MainModule.gcImportFuriYoteiUser) & vbCrLf    '//更新者ＩＤ
        ...
        End If
        sql = sql & " CAUSID = " & gdDBS.ColumnDataSet(MainModule.gcImportFuriYoteiUser) & vbCrLf    '//更新者ＩＤ
        ...
        updCnt = gdDBS.Database.ExecuteSQL(sql)
        '//2007/03/12 マスタ反映のマーキング
        sql = "UPDATE " & mYimp.TfFurikaeImport & " SET " & vbCrLf
        sql = sql & " FIOKFG =  " & mYimp.updReflection & "," & vbCrLf
        sql = sql & " FIUSID = '" & gdDBS.LoginUserName & "'," & vbCrLf
        sql = sql & " FIUPDT = SYSDATE" & vbCrLf
        sql = sql & " WHERE FIINDT = TO_DATE('" & cboImpDate.Text & "','yyyy/mm/dd hh24:mi:ss') " & vbCrLf
        sql = sql & "   AND FISEQN = " & gdDBS.ColumnDataSet(dyn.Fields("FISEQN"), vEnd:=True) & vbCrLf
        updCnt = gdDBS.Database.ExecuteSQL(sql)
        Call dyn.MoveNext
    Loop
    Call dyn.Close
...
#End If
    If Not dyn.EOF Then
        total(0) = gdDBS.Nz(dyn.Fields("OK_CNT"), 0)
        total(1) = gdDBS.Nz(dyn.Fields("NG_CNT"), 0)
        total(2) = gdDBS.Nz(dyn.Fields("DUPCNT"), 0)
    End If
    Call dyn.Close
    Set dyn = Nothing
    '//明細行の合計行が正常分
    sql = "SELECT" & vbCrLf
    sql = sql & " SUM(DECODE(NVL(FIOKFG, 0)," & mYimp.updReflection & ",  NVL(FIICNT,0),0)) OK_CNT," & vbCrLf
    sql = sql & " SUM(DECODE(NVL(FIOKFG, 0)," & mYimp.updReflection & ",0,NVL(FIICNT,0)  )) NG_CNT," & vbCrLf
    sql = sql & " SUM(CASE WHEN NVL(FIOKFG, 0) = " & mYimp.updReflection & " AND NVL(FIICNT,0) > 1 THEN (NVL(FIICNT,0) - 1) "
    sql = sql & "          ELSE 0 END) DUPCNT   " & vbCrLf
    sql = sql & " FROM " & mYimp.TfFurikaeImport & " a " & vbCrLf
    sql = sql & " WHERE FIINDT = TO_DATE('" & cboImpDate.Text & "','yyyy/mm/dd hh24:mi:ss') " & vbCrLf
    ...
    sql = sql & "         AND FIOKFG = " & mYimp.updNormal & vbCrLf
'''    sql = sql & "         AND(FIOKFG = " & mYimp.updNormal & vbCrLf
'''    sql = sql & "               OR(FIOKFG = " & mYimp.updInvalid & vbCrLf
'''    sql = sql & "              AND FIERROR = " & mYimp.errNormal & vbCrLf
'''    sql = sql & "           )" & vbCrLf
'''    sql = sql & "         )" & vbCrLf
    sql = sql & "   )" & vbCrLf
...
#End If
    If Not dyn.EOF Then
        Detail(0) = gdDBS.Nz(dyn.Fields("OK_CNT"), 0)
        Detail(1) = gdDBS.Nz(dyn.Fields("NG_CNT"), 0)
        Detail(2) = gdDBS.Nz(dyn.Fields("DUPCNT"), 0)
    End If
    Call dyn.Close
    Set dyn = Nothing
    '//明細行の合計行が異常分
...
#End If
    ...
    '//マスタ反映時にも同じ事をするので共通化
    If pReflectedMoveTempRecords(gcYoteiImportToMaster) < 0 Then
        GoTo cmdUpdate_ClickError:
    End If
'//2007/06/11 先頭で停止しているのでトリガを再開
    Call gdDBS.TriggerControl("tcHogoshaMaster")
    Call gdDBS.TriggerControl("tfFurikaeYoteiData")
    Call gdDBS.Database.CommitTrans
...
Private Function pDeletedMoveTempRecords(vMode As String) As Long
    Dim sql As String
    '//削除対象データを Temp にバックアップ
    sql = "INSERT INTO " & mYimp.TfFurikaeImport & "Temp" & vbCrLf
    sql = sql & " SELECT SYSDATE,'" & vMode & "',a.*"
    sql = sql & " FROM " & mYimp.TfFurikaeImport & " a " & vbCrLf
    sql = sql & " WHERE FIINDT = TO_DATE('" & cboImpDate.Text & "','yyyy/mm/dd hh24:mi:ss')"
    Call gdDBS.Database.ExecuteSQL(sql)
    sql = "DELETE " & mYimp.TfFurikaeImport & " a " & vbCrLf
    sql = sql & " WHERE FIINDT = TO_DATE('" & cboImpDate.Text & "','yyyy/mm/dd hh24:mi:ss')"
    pDeletedMoveTempRecords = gdDBS.Database.ExecuteSQL(sql)
End Function

Private Function pReflectedMoveTempRecords(vMode As String) As Long
    Dim sql As String, rCnt As Long, dCnt As Long
    '//削除対象データを Temp にバックアップ
    sql = "INSERT INTO " & mYimp.TfFurikaeImport & "Temp" & vbCrLf
    sql = sql & " SELECT SYSDATE,'" & vMode & "',a.*"
    sql = sql & " FROM " & mYimp.TfFurikaeImport & " a " & vbCrLf
    sql = sql & " WHERE                (FIINDT,FISEQN) IN(" & vbCrLf
'//明細の該当する合計行
    sql = sql & "       SELECT DISTINCT FIINDT,FIRKBN " & vbCrLf
    sql = sql & "       FROM " & mYimp.TfFurikaeImport & " b " & vbCrLf
    sql = sql & "       WHERE FIINDT = TO_DATE('" & cboImpDate.Text & "','yyyy/mm/dd hh24:mi:ss') " & vbCrLf
    sql = sql & "         AND FIOKFG = " & mYimp.updReflection & vbCrLf     '//今回反映完了分
    sql = sql & "         AND FIRKBN <> " & mYimp.RecordIsTotal & vbCrLf
    sql = sql & "       UNION " & vbCrLf
'//明細行
    sql = sql & "       SELECT DISTINCT FIINDT,FISEQN " & vbCrLf
    sql = sql & "       FROM " & mYimp.TfFurikaeImport & " a " & vbCrLf
    sql = sql & "       WHERE FIINDT = TO_DATE('" & cboImpDate.Text & "','yyyy/mm/dd hh24:mi:ss') " & vbCrLf
    sql = sql & "         AND FIOKFG = " & mYimp.updReflection & vbCrLf     '//今回反映完了分
    sql = sql & "         AND FIRKBN <> " & mYimp.RecordIsTotal & vbCrLf
    sql = sql & " )"
'//明細のマスタ反映
    rCnt = gdDBS.Database.ExecuteSQL(sql)
    
'//明細の該当する合計行：エラーが１件でもあれば存続：明細削除時に該当
    sql = "DELETE " & mYimp.TfFurikaeImport & " a " & vbCrLf
    sql = sql & " WHERE                (FIINDT,FISEQN) NOT IN(" & vbCrLf
'//明細の該当する合計行：１件でもエラーがある合計行は削除しない
    sql = sql & "       SELECT DISTINCT FIINDT,FIRKBN " & vbCrLf
    sql = sql & "       FROM " & mYimp.TfFurikaeImport & " b " & vbCrLf
    sql = sql & "       WHERE FIINDT = TO_DATE('" & cboImpDate.Text & "','yyyy/mm/dd hh24:mi:ss') " & vbCrLf
    sql = sql & "         AND FIOKFG <> " & mYimp.updReflection & vbCrLf     '//今回反映完了分：削除の合計行は残す！
    sql = sql & "         AND FIRKBN <> " & mYimp.RecordIsTotal & vbCrLf
    sql = sql & "   )"
    sql = sql & "   AND FIRKBN = " & mYimp.RecordIsTotal & vbCrLf
    dCnt = gdDBS.Database.ExecuteSQL(sql)
'//警告分は今回反映完了の対象になっていない！明細の削除は対象になっている
    sql = "DELETE " & mYimp.TfFurikaeImport & " a " & vbCrLf
    sql = sql & " WHERE                (FIINDT,FISEQN) IN(" & vbCrLf
'//明細行
    sql = sql & "       SELECT DISTINCT FIINDT,FISEQN " & vbCrLf
    ...
    sql = sql & "  )"
    dCnt = gdDBS.Database.ExecuteSQL(sql)
    pReflectedMoveTempRecords = rCnt
End Function
...
#If SPR_COL_HIDE = True Then              '//Spreadの非表示列を制御：デバック時のみ
...
Private Sub sprDetail_Change(ByVal Col As Long, ByVal Row As Long)
    ...
    cmdUpdate.Enabled = Not cmdSprUpdate.Enabled
End Sub
...
Private Function pSpreadDetailChange(Optional ByVal Row As Long = -1, Optional vButton As Boolean = False) As Boolean
    ...
    cmdSprUpdate.Enabled = sprTotal.Tag = mSprTotal.RowEdit 'False
    sprDetail.Tag = mSprDetail.RowNonEdit
...
Private Sub sprTotal_Click(ByVal Col As Long, ByVal Row As Long)
    ...
    If False = mLeaveCellEvents And Row > 0 Then
        '//クリックされた合計行の明細を表示する
        Call pSpreadDetailChange(Row, sprDetail.Tag = mSprDetail.RowEdit) ' cmdSprUpdate.Enabled)
    End If
'//2007/03/08 編集不可能な列を色変更
'''    mSprDetail.BackColor(eSprDetail.eImportCnt, -1) = vbRed
'''    mSprDetail.BackColor(eSprDetail.eMochikomiBi, -1) = vbRed
'''    mSprDetail.BackColor(eSprDetail.eMasterKouza, -1) = vbRed
End Sub
...
Private Sub sprTotal_LeaveCell(ByVal Col As Long, ByVal Row As Long, ByVal NewCol As Long, ByVal NewRow As Long, Cancel As Boolean)
    If Row <> NewRow And NewRow > 0 Then
        Cancel = pSpreadDetailChange(NewRow, sprDetail.Tag = mSprTotal.RowEdit)
        '//起動時の１回目のみ LeaveCell イベントが発生しないので制御
...
Private Sub pSpreadRightClick(vRow As Long, vHeader As Boolean)
    ...
    Else
        '//削除対象のＳＥＱ：ヘッダ時は明細も同時に削除
        mDeleteSeqNo = mSprDetail.Value(eSprDetail.eImpSEQ, vRow)
        ...
        mnuSprReset.Caption = ""
        mnuSprDelete.Enabled = mSprDetail.Value(eSprDetail.eErrorFlag, vRow) <> mYimp.errDeleted
        mnuSprReset.Enabled = Not mnuSprDelete.Enabled
    End If
    ...
    Case ePopup.eDelete
        If vHeader = True Then
            ...
            mSprTotal.Text(eSprTotal.eErrorFlag, vRow) = mYimp.errDeleted
        ...
        Else
            ...
            mSprDetail.Text(eSprTotal.eErrorFlag, vRow) = mYimp.errDeleted
    ...
    Case ePopup.eReset
        If vHeader = True Then
            ...
            mSprTotal.Text(eSprTotal.eErrorFlag, vRow) = mYimp.errEditData
        ...
        Else
            ...
            mSprDetail.Text(eSprTotal.eErrorFlag, vRow) = mYimp.errEditData
...
'//2007/03/08 明細の削除見直しをして復活
#If 1 Then  '//2006/11/10 マスタ反映時におかしいのでやめた
Private Sub sprDetail_RightClick(ByVal ClickType As Integer, ByVal Col As Long, ByVal Row As Long, ByVal MouseX As Long, ByVal MouseY As Long)
    If sprDetail.SelBlockRow = Row Then
        If sprDetail.MaxRows > 1 Then
            Call pSpreadRightClick(Row, False)
        Else
            '//2007/03/09 明細が１行ならヘッダ削除ロジックに移行
            Call pSpreadRightClick(sprTotal.ActiveRow, True)
            '//ヘッダを選択してヘッダ＆明細の削除結果を強制表示
            Call pSpreadDetailChange(sprTotal.ActiveRow, cmdSprUpdate.Enabled)
        End If
    End If
End Sub
