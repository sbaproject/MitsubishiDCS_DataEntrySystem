...
Private Type tpHogoshaImport
    ...
    Keiyakusha      As String * 7   '//契約者番号
    Filler          As String * 5   '//予備：教室？に...。
    HogoshaNo       As String * 8   '//保護者番号
    SeitoShimei     As String * 50  '//生徒氏名
    StartYyyyMm     As String * 6   '//開始年月(yyyymm)
...
Private Enum eSprCol
    ...
    eStartYyyyMm    '//開始年月( yyyymmdd ==> yyyy/mm )
...
'// ==> 振替依頼書取込反映.frm(frmFurikaeReqReflectionMode) のラジオボタンと連動しているので注意！
    ...
    Public Sub UpdateMode(vUpdateMode As eUpdateMode, vModeMsg As String)
...
Private Sub pLockedControl(blMode As Boolean, Optional vButton As CommandButton = Nothing)
    ...
    cmdCheck.Enabled = onData 'blMode
    cmdErrList.Enabled = onData 'blMode
    cmdDelete.Enabled = onData 'blMode
    cmdUpdate.Enabled = onData 'blMode
...
#Else
    ...
    sql = sql & "               -1,decode(ciokfg," & mRimp.updInvalid & ",'異常',decode(cimupd,1,'警告','異常'))," & vbCrLf
    sql = sql & "               +0,'正常'," & vbCrLf
    sql = sql & "               +1,'警告'," & vbCrLf
    sql = sql & "               NULL" & vbCrLf
    sql = sql & "           ) || ' => ' || " & vbCrLf
...
'    sql = sql & "    AND TO_CHAR(SYSDATE,'yyyymmdd') BETWEEN BAFKST AND BAFKED" & vbCrLf
    ...
    sql = sql & " DECODE(CIFKST,null,null,substr(CIFKST,1,4) || '/' || substr(CIFKST,5,2)) CIFKST," & vbCrLf
    sql = sql & " DECODE(CIKKBN," & eBankKubun.KinnyuuKikan & ",'民間金融機関'," & eBankKubun.YuubinKyoku & ",'郵便局',NULL)     as CIKKBN," & vbCrLf
...
'//2007/07/19 口座戻りの件数を表示
    ...
    sql = sql & " 　and CIINDT = TO_DATE('" & cboImpDate.Text & "','yyyy/mm/dd hh24:mi:ss')" & vbCrLf
...
Private Function pMoveTempRecords(vCondition As String, vMode As String) As Long
    ...
    sql = sql & vCondition
    result = gdDBS.Database.ExecuteSQL(sql)
    
    sql = "DELETE " & mRimp.TcHogoshaImport & vbCrLf
    sql = sql & " WHERE 1 = 1" & vbCrLf
    sql = sql & vCondition
    result = gdDBS.Database.ExecuteSQL(sql)
    pMoveTempRecords = result
End Function
...
'//2015/02/09 銀行、郵便局両方の記入がある場合 NULL に設定する
            If ("" <> Trim(Hogosha.BankCode) Or "" <> Trim(Hogosha.ShitenCode)) _
            And ("" <> Trim(Hogosha.TuuchoKigou) Or "" <> Trim(Hogosha.TuuchoBango)) Then
...
#End If
    ...
    sql = sql & "CIKYCD = DECODE(CIKYCD,NULL,NULL,LPAD(CIKYCD,7,'0'))," & vbCrLf    '//契約者CD：
    'sql = sql & "CIKSCD = DECODE(CIKSCD,NULL,NULL,LPAD(CIKSCD,3,'0'))," & vbCrLf   '//予備：教室番号
    sql = sql & "CIHGCD = DECODE(CIHGCD,NULL,NULL,LPAD(CIHGCD,8,'0'))," & vbCrLf    '//保護者CD：
    sql = sql & "CIFKST = DECODE(CIFKST,NULL,NULL,LPAD(CIFKST||'01',8,'0'))," & vbCrLf    '//開始年月
...
'//2007/06/12 両方あっても入力が正常であれば？良いだろう。と思ったが？？？
        If IsNull(dynM.Fields("CIKKBN")) Then
            If "" <> gdDBS.Nz(dynM.Fields("CIYBTK")) & gdDBS.Nz(dynM.Fields("CIYBTN")) _
...
#If ORA_DEBUG = 1 Then
Private Function pHogoshaUpdate(vOutDyn As OraDynaset, vInDyn As OraDynaset, vUpdMode As eUpdateMode) As Boolean
#Else
Private Function pHogoshaUpdate(vOutDyn As Object, vInDyn As Object, vUpdMode As eUpdateMode) As Boolean
#End If
    Dim Fields As Variant, ix As Integer
    Dim updSQL As String, chg As Boolean
    Dim sql As String
    Dim result As Long
    Fields = Array("CaITKB", "CaKYCD", "CaHGCD", "CaKJNM", "CaKNNM", "CaSTNM", "CaKKBN", _
                   "CaBANK", "CaSITN", "CaKZSB", "CaKZNO", "CaYBTK", "CaYBTN", "CaKZNM")
    '//入力の相違分のみ更新する
    For ix = LBound(Fields) To UBound(Fields)
        ...
        End If
        If True = chg Then
            updSQL = updSQL & Fields(ix) & " = " & gdDBS.ColumnDataSet(vInDyn.Fields("Ci" & Mid(Fields(ix), 3)), "S") & vbCrLf
        End If
    Next ix
...
#End If
    ...
    If vUpdMode = eUpdateMode.eSouiAddData Then
        If vOutDyn.Fields("CASQNO") = gdDBS.sysDate("YYYYMMDD") Then
    ...
    If vUpdMode = eUpdateMode.eSouiUpdate Then
        ...
        sql = sql & " CAKYST = " & vInDyn.Fields("CiFKST") & "," & vbCrLf
        sql = sql & " CAFKST = " & vInDyn.Fields("CiFKST") & "," & vbCrLf
...
'           sql = sql & " CAKYDT = NULL," & vbCrLf
    ...
    ElseIf vUpdMode = eUpdateMode.eSouiAddData Then
        ...
        sql = sql & " CAKYED = " & gdDBS.LastDay(vInDyn.Fields("CiFKST"), -1) & "," & vbCrLf
        sql = sql & " CAFKED = " & gdDBS.LastDay(vInDyn.Fields("CiFKST"), -1) & "," & vbCrLf
    Else
        '//有り得ない
    ...
    sql = sql & " WHERE CAITKB = " & gdDBS.ColumnDataSet(vOutDyn.Fields("CAITKB"), vEnd:=True) & vbCrLf
    sql = sql & "   AND CAKYCD = " & gdDBS.ColumnDataSet(vOutDyn.Fields("CAKYCD"), vEnd:=True) & vbCrLf
    ...
    If vUpdMode = eUpdateMode.eSouiUpdate Then
        If vOutDyn.Fields("CAFKST") <> vInDyn.Fields("CiFKST") Then
            Call pRirekiAdjust(vOutDyn, vInDyn.Fields("CiFKST").Value)
        End If
    End If
    '//保護者レコードが存在して追加モードでした場合、新規レコード追加
    If vUpdMode = eUpdateMode.eSouiAddData Then
        pHogoshaUpdate = pHogoshaInsert(vInDyn)
    Else
        pHogoshaUpdate = True
    End If
End Function
...
Private Sub pRirekiAdjust(vDyn As OraDynaset, vCiFKST As String)
    Dim sql As String, dyn As OraDynaset
    sql = "SELECT * FROM tcHogoshaMaster"
    sql = sql & " WHERE CAITKB = " & gdDBS.ColumnDataSet(vDyn.Fields("CAITKB"), vEnd:=True) & vbCrLf
    sql = sql & "   AND CAKYCD = " & gdDBS.ColumnDataSet(vDyn.Fields("CAKYCD"), vEnd:=True) & vbCrLf
    sql = sql & "   AND CAHGCD = " & gdDBS.ColumnDataSet(vDyn.Fields("CAHGCD"), vEnd:=True) & vbCrLf
    sql = sql & "   AND CASQNO < " & gdDBS.ColumnDataSet(vDyn.Fields("CASQNO"), "L", vEnd:=True) & vbCrLf
    ...
    chgDate = gdDBS.LastDay(vCiFKST, -1)
    ...
    sql = sql & " WHERE CAITKB = " & gdDBS.ColumnDataSet(dyn.Fields("CAITKB"), vEnd:=True) & vbCrLf
    sql = sql & "   AND CAKYCD = " & gdDBS.ColumnDataSet(dyn.Fields("CAKYCD"), vEnd:=True) & vbCrLf
    sql = sql & "   AND CAHGCD = " & gdDBS.ColumnDataSet(dyn.Fields("CAHGCD"), vEnd:=True) & vbCrLf
    sql = sql & "   AND CASQNO = " & gdDBS.ColumnDataSet(dyn.Fields("CASQNO"), "L", vEnd:=True) & vbCrLf
    result = gdDBS.Database.ExecuteSQL(sql)
End Sub
...
Private Sub cmdUpdate_Click()
    ...
    End If
    Dim frm As Form
    Set frm = frmFurikaeReqReflectionMode
...
#End If
    ...
    If dyn.EOF Then
        ...
        Call gdDBS.AutoLogOut(mCaption, Replace(msg, vbCrLf, ""))
        Call MsgBox(msg, vbOKOnly + vbInformation, mCaption)
        '//ボタンを戻す
        cmdUpdate.Caption = cBtnUpdate
        '//コマンド・ボタン制御
        Call pLockedControl(True)
...
'// Call gdDBS.TriggerControl("tcHogoshaMaster", False)
    ...
    startTimeSQL = startTimeSQL & " AND CIINDT = TO_DATE('" & cboImpDate.Text & "','yyyy/mm/dd hh24:mi:ss') " & vbCrLf
    ...
    startTimeSQL = startTimeSQL & " WHERE CaUPDT >= TO_DATE('" & gdDBS.sysDate("yyyy/mm/dd hh24:mi:ss") & "','yyyy/mm/dd hh24:mi:ss') " & vbCrLf
    ...
    Do Until dyn.EOF
        ...
        sql = sql & " WHERE CAITKB = " & gdDBS.ColumnDataSet(dyn.Fields("CIITKB"), vEnd:=True)
        sql = sql & "   AND CAKYCD = " & gdDBS.ColumnDataSet(dyn.Fields("CIKYCD"), vEnd:=True)
...
#End If
        If updDyn.EOF Then
            ...
            End If
        Else
            If False = pHogoshaUpdate(updDyn, dyn, mUpdateMode) Then
                GoTo cmdUpdate_ClickError:
            End If
    ...
    Set dyn = Nothing
    '//マスタ反映時にも同じ事をするので共通化
    If pMoveTempRecords(startTimeSQL, gcFurikaeImportToMaster) < 0 Then
        GoTo cmdUpdate_ClickError:
    End If
    Call gdDBS.Database.CommitTrans
...
Private Sub Form_Load()
    ...
    '//これはフレーム
    fraProgressBar.Top = stbStatus.Top + 15
...
Private Sub Form_Resize()
    ...
    If Me.Height < cmdImport.Top + 1700 Then
        Me.Height = cmdImport.Top + 1700
    End If
    If Me.Width < sprMeisai.Width + sprMeisai.Left * 3 Then
        Me.Width = sprMeisai.Width + sprMeisai.Left * 3
    End If
    Call mForm.Resize
        ...
        Or TypeOf obj Is Label Then
            ...
            If "" <> obj.DataField And True = obj.DataChanged Then
                ...
                Case "CIINSD" '//eAddMode           '2014/05/19 追加更新
                    mSpread.Value(eSprCol.eAddMode, mEditRow) = gdFormSub.lblCiINSD.Caption
...
Private Sub sprMeisai_DblClick(ByVal Col As Long, ByVal Row As Long)
    ...
    'gdFormSub.lblCiINSDAT.DataChanged = False
    Call mForm.ResetDataControlEditFlag(gdFormSub)
...
'    eKouzaName      '口座名義人=>保護者名(カナ)
    ...
    ErrStts = Array("CIERROr", "CIITKBe", _
                    "CIKYCDe", "cikycde", "CIHGCDe", "CIKJNMe", "CIKNNMe", "CISTNMe", "CIFKSTe", _
...
#End If
    ...
    Do Until dyn.EOF
        ...
        For ix = LBound(ErrStts) To UBound(ErrStts)
            '//各列の表示色変更
            mSpread.BackColor(ix + 2, dyn.RowPosition) = mRimp.ErrorStatus(dyn.Fields(ErrStts(ix)))
        Next ix
        '//処理結果列の表示色
        '//2006/04/04 マスタ反映ＯＫフラグ判断
        '//2014/06/09 異常データはそのままの色でおいておく
        If mRimp.updInvalid() <> Val(dyn.Fields("CiOKFG")) Then
